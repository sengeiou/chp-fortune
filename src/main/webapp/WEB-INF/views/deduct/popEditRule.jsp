		...
		<table class="f14 table4" cellpadding="0" cellspacing="0" border="0" width="100%">
				...
				<td>
					...
					<select name="dictDeductPlatformId" class="cf_input_text178">
						...
						<c:forEach items="${fns:getDictList('tz_deduct_plat')}" var="item">
							<option value="${item.value}" <c:if test="${rule.dictDeductPlatformId==item.value}">selected</c:if>>${item.label}</option>
						</c:forEach>
					</select>
				</td>
			</tr>
				...
				<td>
					...
					<select name="dictBankId" class="cf_input_text178 ">
						...
						<c:forEach items="${fns:getDictList('tz_open_bank') }" var="item">
							<option value="${item.value}" <c:if test="${rule.dictBankId==item.value}">selected</c:if>>
								${item.label}
							</option>
						</c:forEach>
					</select>
			...
			<tr>
				<td>
					...
					<input type="text" name="singleLimitMoney" value="${rule.singleLimitMoney }"/>
				</td>
			</tr>
			<tr>
				<td>
					<label class="lab">每日限额：</label>
					<input type="text" name="dayLimitMoney" value="${rule.dayLimitMoney }"/>
				</td>
			</tr>
			<tr>
				<td>
					...
					<c:forEach items="${fns:getDictList('tz_yes_no')}" var='item'>
						<input type="radio" name="isFirst" value="${item.value}" <c:if test="${rule.isFirst==item.value}">checked</c:if>>${item.label}
					</c:forEach>
				</td>
			</tr>
			<tr>
				<td>
					...
					<c:forEach items="${fns:getDictList('tz_deductinter_type')}" var='item'>
						<input type="radio" name="dictDeductInterfaceType" value="${item.value}" <c:if test="${rule.dictDeductInterfaceType==item.value}">checked</c:if>>${item.label}
					</c:forEach>
				</td>
			</tr>
		</table>
