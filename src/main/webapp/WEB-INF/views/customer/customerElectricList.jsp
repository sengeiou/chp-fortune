...
<head>
	...
	<script src="${ctxWebInf}/js/customer/customerElectricList.js" type="text/javascript"></script>
</head>
<body>
	<div class="body_r">
	<div class="box1 mb10">
	<form id="searchForm" action="${ctx}/customer/investment/listCustomerElectric?menuId=${menuId}" method="post" >
		...
		<sys:message content="${message}"/>
		 <table class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
			...
			<tr>
				<td>
					<label class='lab'>原营业部：</label>
					<sys:allOrgTree id="org"  name="customerOldOrgId" value="${customer.customerOldOrgId}" labelValue="${customer.customerOldOrgName}" labelName="customerOldOrgName"/>
				</td>
				<td>
					<label class='lab'>原城市：</label>
	...
	<auth:hasPermission key="cf:customer:listCustomerElectricExport">
...
<!-- 				<th>电销理财专员</th> -->
		...
		<c:forEach items="${page.list}" var="item" varStatus="sta">
			<tr>
				...
				<!-- 屏蔽客户姓名/手机号/身份证号 -->
				<td>***</td>
				<%-- <td>${item.custMobilephone}</td> --%>
				<td>${fns:getProvinceLabel(item.customerOldProvinceId) }</td>
				<td>${fns:getCityLabel(item.customerOldCityid) }</td>
				<td>${item.customerOldOrgName }</td>
...
<%-- 				<td>${item.teleManagerName }</td> --%>
			...
			<%-- 	<td>
					<c:if test="${item.transferState!=1}">
						...
						<auth:hasOwner managerCode="${item.managerCode }"  > 	
							<c:if test="${item.custLending==1 and empty item.custCertNum}">
