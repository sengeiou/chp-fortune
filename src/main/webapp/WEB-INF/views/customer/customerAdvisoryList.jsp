...
<script type="text/javascript" src="${ctxWebInf}/js/customer/customerAdvisoryList.js" charset="utf-8""></script>
<p class='tright pr30'>
<c:if test="${belongToCurrentUser }">
<input id="btnAddAdvisory" class="btn btn_sc" type="button" value="新增"/></c:if>
		...
		<c:if test="${fn:length(page.list)>0 }">
			<c:forEach items="${page.list}" var="item">
				<tr>
					...
					<td>${item.askDes }</td>
					<td>
						<input id="advisoryDetail" class="cf_btn_edit" type="button" value="详细" advisoryId="${item.id }">
					</td>
				</tr>
			</c:forEach>
		</c:if>
