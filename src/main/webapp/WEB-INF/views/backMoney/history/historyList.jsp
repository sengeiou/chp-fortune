...
<script type="text/javascript" src="${ctxWebInf}/js/backMoney/history.js"></script>
<script type="text/javascript" src="${ctxWebInf}/js/common/subdialog.js"></script>
<title>回款已办列表</title>
</head>
<body>
<div class="body_r">
    <form id="searchForm" action="${ctx}/myDone/backMoney/historyList"  method="post">
    <div class="box1 mb10">
    ...
    <p class="mb10">
    	<auth:hasPermission key="cf:backmoneyhistory:batchback">
			<input type="button" onclick="javascript:multiReturn();" class="btn btn_sc ml10" value="批量退回">
		</auth:hasPermission>
		<auth:hasPermission key="cf:backmoneyhistory:exportexect">
...
<%-- 	    <input type="button" onclick="javascript:exportExcel('${ejUrl}');" class="btn btn_sc" value="待金账户回款明细"> --%>
	...
	<div class='box5'>
		<sys:columnCtrl pageToken="backMoney_history_historyList"></sys:columnCtrl>
	    <table id="listTable" class="table table-striped table-bordered table-condensed data-list-table">
	        ...
	        <c:forEach items="${page.list}" var="item">
		        <tr>
		            ...
		            <td>
		            	 <c:if test="${item.isSupplemented==1 }">是</c:if>
		            	 <c:if test="${item.isSupplemented==2 }">否</c:if>
		            </td>
		            <td>${fns:dictName(dicts.tz_open_bank,item.backBank,item.backBank) }</td>
		            <td>${fns:dictName(dicts.tz_working_state,item.workingState,'-') }</td>
		            <td class="tcenter">
		            	<auth:hasPermission key="cf:backmoneyhistory:transact">
				            <a href="${ctx}/myDone/backMoney/historyDetail?backmId=${item.backmId }">
				            	办理
				            </a>
				        </auth:hasPermission>
		            </td>
