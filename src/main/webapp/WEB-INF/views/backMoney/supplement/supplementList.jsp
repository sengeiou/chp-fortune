...
<body>
<div class="body_r">
    <form id="searchForm" action="${ctx}/myTodo/backMoney/supplementList" method="post">
    <div class="box1 mb10">
    ...
    <p class="mb10 ml10">
    	<auth:hasPermission key="cf:backmoneysupplement:batchbackmoneysupplement">
       		<input type="button" onclick="multiSupplement();" class="btn btn_sc ml10" value="批量补息确认"/>
       	</auth:hasPermission>
       	<auth:hasPermission key="cf:backmoneysupplement:backmoneydetail">
       		<input type="button" onclick="javascript:exportExcel('${eUrl}');" class="btn btn_sc" value="导出回款明细"/>
       	</auth:hasPermission>
       	<auth:hasPermission key="cf:backmoneysupplement:goldbackmoneydetail">
       		<input type="button" onclick="javascript:exportExcel('${ejUrl}');" class="btn btn_sc" value="待金账户回款明细"/>
       	</auth:hasPermission>
       	<auth:hasPermission key="cf:backmoneysupplement:uploadbackmoney">
       		<input type="button" onclick="javascript:uploadExcel();" class="btn btn_sc" value="上传回款金额"/>
       	</auth:hasPermission>
       	<auth:hasPermission key="cf:backmoneysupplement:updatedebtdate">
       		<input type="button" onclick="javascript:updatedebtdate();" class="btn btn_sc" value="修改债转日"/>
       	</auth:hasPermission>
	    ...
	    <table id="listTable" class="table table-striped table-bordered table-condensed data-list-table">
	        ...
	        <c:forEach items="${page.list}" var="item">
		        <tr>
		            ...
		            <td class="tcenter">
		            	<auth:hasPermission key="cf:backmoneysupplement:transact">
			            	<a href="${ctx}/myTodo/backMoney/supplementDetail?backmId=${item.backmId }">
			            		办理
			            	</a>
			            </auth:hasPermission>
		            </td>
