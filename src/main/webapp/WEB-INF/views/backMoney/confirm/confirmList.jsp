...
<script type="text/javascript" src="${ctxWebInf}/js/backMoney/confirm.js"></script>
<title>回款确认列表</title>
</head>
<body>
<div class="body_r">
    <form id="searchForm" action="${ctx}/myTodo/backMoney/confirmList" method="post" >
    <div class="box1 mb10">
        ...
        <table class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
            ...
            <tr id="T3" style="display:none">
                ...
                <td><label class="lab">回款平台：</label>
                	<select class="select78" id='platformIds' name='platformIds' multiple="multiple">
							<c:forEach items="${dicts.tz_backMoney_plat}" var='item'>
								<option value="${item.value }" <c:if test="${fns:multiOption(view.platformIds,item.value) }">selected</c:if>>
									${item.label }
								</option>
							</c:forEach>
					</select>
            ...
            <tr id="T4" style="display:none">
                <td><label class="lab">回盘结果：</label>
                	<select class="select78" id='backResult' name='backResult' multiple="multiple">
							<c:forEach items="${dicts.tz_back_result}" var='item'>
								<option value="${item.value }" <c:if test="${fns:multiOption(view.backResult,item.value) }">selected</c:if>>
									${item.label }
								</option>
							</c:forEach>
					</select>
    ...
    <p class="mb10">
    	<auth:hasPermission key="cf:backmoneyconfirm:batchconfirm">
    		<input type="button" onclick="multiConfirm();" class="btn btn_sc ml10" value="批量确认"/>
    	</auth:hasPermission>
    	<auth:hasPermission key="cf:backmoneyconfirm:backbackmoney">
    		<input type="button" onclick="multiBacktoExecute();" class="btn btn_sc" value="退回到待回款"/>
    	</auth:hasPermission>
    	<auth:hasPermission key="cf:backmoneyconfirm:exportExcel">
			<input type="button" onclick="javascript:exportExcel('${eUrl}');" class="btn btn_sc" value="导出EXCEL">
		</auth:hasPermission>
		<auth:hasPermission key="cf:backmoneyconfirm:uploadExcel">
			<input type="button" onclick="uploadExcel();" class="btn btn_sc" value="上传回盘结果">
		</auth:hasPermission>
<%-- 	    <input type="button" onclick="javascript:exportExcel('${ejUrl}');" class="btn btn_sc" value="待金账户回款明细"> --%>
    ...
    <div class='box5'>   
    <sys:columnCtrl pageToken="backMoney_confirm_confirmList"></sys:columnCtrl>
    <table id="listTable" class="table table-striped table-bordered table-condensed data-list-table tableList">
        <thead>
        <tr>
            <th>
        ...
        <c:forEach items="${page.list}" var="item">
	        <tr>
	            ...
	            <td>${fns:dictName(dicts.tz_back_type,item.backMoneyType,'-') }</td>
	            <td>${fns:getFormatNumber(item.successMoney ,'￥#,##0.00')}</td>
	            <td>${fns:getFormatNumber(item.failMoney ,'￥#,##0.00')}</td>
	            <td>${fns:dictName(dicts.tz_back_result,item.backResult,'') }</td>
	            <td>${fns:getFormatDate(item.backResultDay,"yyyy-MM-dd")}</td>
	            <td>${item.backReason}</td>
	            <td>${fns:dictName(dicts.tz_working_state,item.workingState,'-') }</td>
	            <td class="tcenter">
	            	<auth:hasPermission key="cf:backmoneyconfirm:transact">
			            <a href="${ctx}/myTodo/backMoney/confirmDetail?backmId=${item.backmId }">
			            	办理
			            </a>
			        </auth:hasPermission>
	            </td>
