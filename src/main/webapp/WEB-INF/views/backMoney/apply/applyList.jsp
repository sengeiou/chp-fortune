...
<script type="text/javascript" src="${ctxWebInf}/js/backMoney/apply.js"></script>
<title>回款申请列表</title>
</head>
<body>
<div class="body_r">
    <form id="searchForm" action="${ctx}/myApply/backMoney/applyList" method="post">
    <div class="box1 mb10">
        ...
        <table class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
            ...
            <tr id="T5" style="display:none">
            	<td>
					...
					<select name="priorityBack" class="select78">
                	<option value="">请选择</option>
                	<c:forEach  items="${dicts.tz_priority_back}" var="item">
							<option value="${item.value }"
								<c:if test="${fns:multiOption(view.priorityBack, item.value)}">
									selected
								</c:if>>${item.label }
							</option>
					</c:forEach>
    ...
    <p class='ml10'>
    	<auth:hasPermission key="cf:backmoneyapply:batchbackmoneyapply">
		    <input type="button" onclick="javascript:multiApply();" class="btn btn_sc ml10" value="批量回款申请">
		</auth:hasPermission>
		<auth:hasPermission key="cf:backmoneyapply:backmoneyapplydetail">
		    <input type="button" onclick="javascript:exportExcel('${eUrl}');" class="btn btn_sc" value="待回款申请明细">
		</auth:hasPermission>
		<auth:hasPermission key="cf:backmoneyapply:goldbackmoneyapplydetail">
		    <input type="button" onclick="javascript:exportExcel('${ejUrl}');" class="btn btn_sc" value="待金账户回款明细">
		</auth:hasPermission>
		<auth:hasPermission key="cf:backmoneyapply:backmoneyapplychanneldetail">
		    <input type="button" onclick="javascript:updateChannel();" class="btn btn_sc" value="上传渠道标识">
		</auth:hasPermission>
		    <input type="hidden" id="hTotalLendMoney" value="${fns:getFormatNumber(totalLM,'##0.00')}">
		    <input type="hidden" id="hTotalBackMoney" value="${fns:getFormatNumber(totalBM,'##0.00')}">
		    <input type="hidden" id="hDataAmount" value="${page.count}">
	<label>
	累计出借金额 ：￥<label id="TotalLendMoney">${fns:getFormatNumber(totalLM,'##0.00')}</label>元     
	累计应回款金额 ：￥<label id="TotalBackMoney">${fns:getFormatNumber(totalBM,'##0.00')}</label>元     
	累计数量 ：<label id="dataAmount">${page.count}</label>条  </label>
	</p>
	<div class='box5'>	    	
	<sys:columnCtrl pageToken="backMoney_apply_applyList"></sys:columnCtrl>
        ...
        <c:forEach items="${page.list}" var="item">
	        <tr>
	            ...
	            <td>
	            	...
	            	<input type="hidden" name="itemApplyLendMoney" value="${fns:getFormatNumber(item.applyLendMoney ,'##0.00')}"/>
	            </td>
	            ...
	            <td>
	            	${fns:getFormatNumber(item.backMoney ,'￥#,##0.00')}
	            	<input type="hidden" name="itemBackMoney" value="${fns:getFormatNumber(item.backMoney ,'##0.00')}">
	            </td>
	            <td>
	            ...
	            <td class="tcenter">
	            	<auth:hasPermission key="cf:backmoneyapply:transact">
		            	<a href="${ctx}/myApply/backMoney/applyDetail?backmId=${item.backmId }" class='cf_btn_edit'>
		            		办理
		            	</a>
		            </auth:hasPermission>
	            </td>
