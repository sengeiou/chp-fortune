...
<script src="${ctxWebInf}/js/delivery/deliveryRemindList.js" type="text/javascript"></script>
<title>客户交割</title>
</head>
<body>
<div class="body_top body_r">
  <div class="box1 mb10"> 
  <form:form id="searchForm" modelAttribute="deliveryEx" action="${ctx}/delivery/customer/deliveryRemindList" method="post">
    <table class="table1" cellpadding="0" cellspacing="0" border="0" width="100%">
            ...
            <tr id="T2" style="display:none">
                ...
                <input type="text" class="Wdate input_text70" id="startTime" name="startTime" onfocus="WdatePicker({maxDate:'#F{$dp.$D(\'endTime\')}'})"  value="${fns:getFormatDate(deliveryEx.startTime ,'yyyy-MM-dd')}"/> -
                <input type="text" class="Wdate input_text70" id="endTime" name="endTime" onfocus="WdatePicker({minDate:'#F{$dp.$D(\'startTime\')}'})" value="${fns:getFormatDate(deliveryEx.endTime ,'yyyy-MM-dd')}"/>
                </td>
            </tr>
        </table>
		...
		<div class="mb10">
    	     ...
    	     <button class="btn btn_sc" id="outExcel">导出Excel</button>&nbsp;&nbsp;总笔数：${page.count}条
        </div>
              ...
              <tbody>
                <c:forEach items="${page.list}" var="item" varStatus="status">
                  <tr>
                    ...
                    <td>${item.createBy}</td>
                    <td>
                        ${fns:getFormatDate(item.delDate,'yyyy-MM-dd')}
                        <input type="hidden" id="delId${status.count}" value="${item.delId}"/>
                    </td>
                    <td>${fns:getDictLabel(item.dictDelStatus, 'tz_delivery_status', '-')}</td>
                    <td><input type="text" style="width:70px" class="Wdate select78_24" id="delDate${status.count}" onfocus="WdatePicker()"></td>
                    <td nowrap="nowrap">
