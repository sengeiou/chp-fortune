...
<script type="text/javascript" src="${ctxWebInf}/js/backInterest/returnUploadMoney.js"></script>
<script type="text/javascript" src="${ctxWebInf}/js/backInterest/backReturnInterestApplyConfrim.js"></script>

</head>
<body>
	<div class="body_r">
		<div class="box1 mb10">
			<form id="searchForm" action="${ctx}/backReturnInterestApplyConfrim/loadBackReturnInterestApplyConfrimList" method="post">
		...
		<p class="mb10">
			<auth:hasPermission key="cf:backreturninterestapplyconfirm:backreturninterestapplydetail">
				<input type="button" class="btn btn_sc ml10" value="导出EXCEL" onclick="applyExport('backReturnInterestApplyConfrim/exportReturnExl');" />
			</auth:hasPermission>
			<auth:hasPermission key="cf:backreturninterestapplyconfirm:uploadbackreturninterest">
				<input type="button" class="btn btn_sc" value="上传回息金额" onclick="uploadMoney()" />
			</auth:hasPermission>
			<auth:hasPermission key="cf:returninterestapplyconfirm:batchreturninterestapplyconfirm">
				<input type="button" class="btn btn_sc" value="批量回息申请确认" onclick="returnBatchApprovalWindow()" />
			</auth:hasPermission>
			<auth:hasPermission key="cf:returninterestapplyconfirm:goldreturninterestapplydetail">
				<input type="button" class="btn btn_sc" value="待金账户回息明细" onclick="applyExport('backReturnInterestApplyConfrim/goldAccountExportReturnExl')" />
			</auth:hasPermission>
			...
			<sys:columnCtrl pageToken="return_backInterest_backInterestApplyConfrimList"></sys:columnCtrl>
		<div class='box5'>
			<table class="table table-striped table-bordered table-condensed data-list-table">
				...
				<c:forEach items="${bibv.page.list}" var="back" varStatus="status">
					<tr>
						...
						<td align="center">${fns:dictName(dicts.tz_back_interest_return,back.isInterest,'-')}</td>
						<td align="center">
							<auth:hasPermission key="cf:backreturninterestapplyconfirm:goreturnconfrim">
								<input class='cf_btn_edit' type="button" value="办理" onclick="window.location.href='${ctx}/backReturnInterestApplyConfrim/goReturnConfrim?code=${back.backiId}'" />
							</auth:hasPermission>
						</td>
					</tr>
				</c:forEach>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="tright pr30">
					<input type="button" value="提交" class="btn cf_btn-primary" onclick="javascript:applyConfrim('backReturnInterestApplyConfrim/batchApplyReturnConfirm','backReturnInterestApplyConfrim/loadBackReturnInterestApplyConfrimList');" />
