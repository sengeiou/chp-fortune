...
<div class="body_r">
    ...
    <div class="box1 mb10">
    	<form id="searchForm"  action="${ctx}/backInterestApply/loadBackInterestApplylist" method="post">
    ...
    <p class="mb10 ml10">
    	<auth:hasPermission key="cf:backinterestapply:exportExcel">
    		<input type="button" value="导出EXCEL" onclick="applyExport('backInterestApply/exportExl')" class="btn btn_sc ml10"/>
    	</auth:hasPermission>
    	<auth:hasPermission key="cf:backinterestapply:batchbackinterestapply">
    		<input type="button" value="批量回息申请" class="btn btn_sc" onclick="batchApply();"/>
    	</auth:hasPermission>
    	<auth:hasPermission key="cf:backinterestapply:goldbackinterestapplydetail">
    		<input type="button" value="待金账户回息明细" id="account" onclick="applyExport('backInterestApply/goldAccountExportExl')" class="btn btn_sc"/>&nbsp;
    	</auth:hasPermission>
    ...
    <sys:columnCtrl pageToken="backInterest_backInterestApplyList"></sys:columnCtrl>
    <div class='box5'>
			...
			<c:forEach items="${bibv.page.list}" var="back" varStatus="status">
				<tr>
					...
					<td align="center">${fns:dictName(dicts.tz_backsms_state,back.backMoneyStatus,'-')}</td>
       				<td align="center">
       					<auth:hasPermission key="cf:backinterestapply:transact">
       						<input type="button" class="cf_btn_edit" value="办理" onclick="window.location.href='${ctx}/backInterestApply/toApply?backiId=${back.backiId}'"/>
       					</auth:hasPermission>
       				</td>
				</tr>
			</c:forEach>
