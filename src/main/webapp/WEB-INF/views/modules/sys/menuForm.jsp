...
<body>
	...
	<form:form id="inputForm" modelAttribute="menu" action="${ctx}/sys/menu/save" method="post" class="form-horizontal">
		...
		<div class="control-group">
			<label class="control-label">所属模块菜单:</label>
			<div class="controls">
			    <c:if test="${disabled eq true }">
			    	<input type="hidden" id="moduleMenuId" name="moduleMenu.Id" value="${menu.moduleMenu.id }"/>
			    </c:if>
				<select id="moduleMenuId" name="moduleMenu.Id" <c:if test="${disabled eq true }">disabled</c:if>>
	                <c:forEach var="item" items="${moduleMenuList}" varStatus="status">
	                    <option value="${item.id }" <c:if test="${item.id eq menu.moduleMenu.id }">selected</c:if>>${item.name}</option>
	                </c:forEach>
		        </select>
			</div>
		</div>
		<div class="control-group">
			...
			<div class="controls">
                <sys:menuselect id="menu" name="parent.id" value="${menu.parent.id}" labelName="parent.name" labelValue="${menu.parent.name}"
					title="菜单" url="/sys/menu/treeData" extId="${menu.id}" mmId="${menu.id}" cssClass="required"/>
			</div>
		</div>
		<div class="control-group">
