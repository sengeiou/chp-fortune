...
<body>
	...
	<form:form id="searchForm" action="${ctx}/sys/menu/" method="post">
		<div>
			<label>模块菜单：</label>
			<select id="moduleMenuId" name="moduleMenuId">
                <c:forEach  var="item" items="${moduleMenuList}" varStatus="status">
                    <option value="${item.id }" <c:if test="${item.id eq searchView.moduleMenuId }">selected</c:if>>${item.name}</option>
                </c:forEach>
		    </select>
			<input id="btnSubmit" class="btn cf_btn-primary" type="submit" value="查询"/>
		</div>
	</form:form>
	...
	<form id="listForm" method="post">
		<table id="treeTable" class="table table-striped table-bordered table-condensed hide">
			...
			<tbody><c:forEach items="${list}" var="menu">
				<tr id="${menu.id}" pId="${menu.parent.id ne rootMenu.id?menu.parent.id:'0'}">
					...
					<shiro:hasPermission name="sys:menu:edit"><td nowrap>
						<a href="${ctx}/sys/menu/form?id=${menu.id}&moduleMenu.id=${menu.moduleMenu.id}&flag=2">修改</a>
						<a href="${ctx}/sys/menu/delete?id=${menu.id}" onclick="return confirmx('要删除该菜单及所有子菜单项吗？', this.href)">删除</a>
						<a href="${ctx}/sys/menu/form?parent.id=${menu.id}&moduleMenu.id=${menu.moduleMenu.id}&flag=2">添加下级菜单</a> 
					</td></shiro:hasPermission>
				</tr>
			</c:forEach></tbody>
		</table>
		<shiro:hasPermission name="sys:menu:edit"><div class="form-actions pagination-left">
			<input id="btnSubmit" class="btn cf_btn-primary" type="button" value="保存排序" onclick="updateSort();"/>
