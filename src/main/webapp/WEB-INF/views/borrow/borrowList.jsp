...
<body>
		...
		<form action="${ctx}/borrow/list" method="post" id="searchForm">
			<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
			<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
			<input id="creditValueIdList" name="creditValueIdList" type="hidden"/>
			<input id="hiddenMoney" type="hidden" value="${fns:getFormatNumber(allMoneyOneByOne,'0.00')}"/>
			<input id="hiddenMoneyOneByOne"  type="hidden" value="${fns:getFormatNumber(allMoney,'0.00')}"/>
				...
				<tr id="T2" style="display: none;">
					...
					<td>
	                	...
	                	<select name="zqState" class="select78 mr34" multiple="multiple">
	                		...
	                		<option value="0" <c:if test="${borrowView.zqState == 0}">selected</c:if>>-</option>
	                		<option value="1" <c:if test="${borrowView.zqState == 1}">selected</c:if>>    </option>
	                     </select>
	                </td>
				</tr>
				</table>
		...
		<p class="mb10 ml10">
			<auth:hasPermission key="cf:borrow:exportexcel">
				<button class="btn btn_sc ml10" onclick="outExcel()">导出EXCEL</button>
			</auth:hasPermission>
			<auth:hasPermission key="cf:borrow:exportBorrow">
				<button class="btn btn_sc" onclick="exportBorrow()">导出债权Excel</button>
			</auth:hasPermission>
			<auth:hasPermission key="cf:borrow:batchallot">
				<button class="btn btn_sc" onclick="borrowBatchAlot()">批量分配</button>
			</auth:hasPermission>
			<auth:hasPermission key="cf:borrow:uploadborrow">
				<button id="borrowUpload" class="btn btn_sc">债权上传</button>
			</auth:hasPermission>
			<auth:hasPermission key="cf:borrow:pushBorrow">
			<button id = "pushBorrowButtun" class="btn btn_sc" onclick="pushBorrow()">推送债权至债权记录</button>
			</auth:hasPermission>
			总金额：￥<span id="money">${fns:getFormatNumber(allMoney,'#,##0.00')}</span>元 总笔数：<span id="count">${page.count }</span>笔
			总金额（L2）：￥<span id="moneyOneByOne">${fns:getFormatNumber(allMoneyOneByOne,'#,##0.00')}</span>元
		</p>
		<div id="content-body">
				...
				<c:forEach items="${page.list }" var="item" varStatus="s">
					<tr>
						<td id="creditValueIdCheckbox">
	           			<input type="checkbox" name='creditValueId' value="${item.creditValueId}:${item.verTime}:${item.creditMonId}:${item.borrowMonthVerTime}" id="checkOne">
						${s.index+1}
						</td>
						<!-- 屏蔽客户姓名/手机号/身份证号 -->
						<td>***</td>
						...
						<input id="loanCreditValueOneByOne" type="hidden" value = "${item.loanCreditValueYS}"/>
						...
						<td>
							<auth:hasPermission key="cf:borrow:transact">
								<input type="button" value="办理" class="cf_btn_edit" onclick="goPage('${ctx}/borrow/goBorrowLook?creditValueId=${item.creditValueId}');">
							</auth:hasPermission>
						</td>
					</tr>
				</c:forEach>
