...
//    	url = '/theDayDeduct/onLineDeductShow';
//    	argment = null;
...
//        });
	...
	$("#downLineDeduct").click(function() {
			...
			// 调用子窗口的参数
			var url, argment, callback;
			url = '/theDayDeduct/offLineDeductShow';
			argment = null;
			load_callback = function(arg){
	...
	$("#protocolLibrary").click(function() {
		...
		BootstrapDialog.confirm("您确定要协议库对接吗？", function(result) {
			if (result) {
				...
				var url = ctx + "/theDayDeduct/protocolLibrary";
	...
	$("#bathDeductBalance").click(function() {
		BootstrapDialog.confirm("您确定要批量修改划扣状态吗？",function(result) {
				if (result) {
					...
					$("#subClose").val("提交");
					var url, argment, callback;
					url = '/theDayDeduct/showBatchModel';
					argment = null;
					load_callback = function(arg){
					...
					$('#subClose').off('click').on('click',function(){
						...
						// 异步提交表单
						url = ctx + '/theDayDeduct/theayBatchStatus';
	...
	$("#bathDeductTheDay").click(function(){
		BootstrapDialog.confirm("您确定要预约划扣吗？",function(result) {
			if (result) {
				...
				//自定义出发关闭回调函数的动作
				$('#subClose').off('click').on('click',function(){
					url = ctx+"/theDayDeduct/saveDeductBespoke";
...
//				$(".modalDiv").attr("style","display:none");
				$("#subClose").val("提交");
				var url, argment, callback;
				url = '/theDayDeduct/goCancelDeductBespoke';
				argment = {};
				load_callback = function(arg){
					$("input[type='button']").click(function(){
						url = ctx+"/theDayDeduct/cancelDeductBespoke";
						contents_getJsonForSync(url, {id:getCancelIds()}, "post", callFunction, errorCal, null);
					});
				};
				close_callback = function(arg){};
				var sub = SubDialogHandle.create(url,argment,load_callback,close_callback).loadSubWindow();
