...
function uploadMoney(){
	...
	if(s == 0){
		...
		url = "/backInterestApplyConfrim/uploadMoneyWindow";
		argment = null;
...
function upload(formData){
	...
	url = ctx+"/backInterestApplyConfrim/uploadMoney";
	data = formData;
	successCal = function(result){
		loadingMarkHide();
		if(result=="true"){
			BootstrapDialog.alert("操作成功",function(){loadingMarkShow();window.location.href=ctx+"/backInterestApplyConfrim/loadBackInterestApplyConfrimList"});
		}else{
			BootstrapDialog.alert(result,function(){loadingMarkShow();window.location.href=ctx+"/backInterestApplyConfrim/loadBackInterestApplyConfrimList"});
       	 }
	};
