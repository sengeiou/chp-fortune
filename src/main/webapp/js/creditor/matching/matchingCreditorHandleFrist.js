...
//				});
  	 			...
  	 			ctx+'/matchingcreditor/getBorrowByCreditValueIds?creditValueIds='+creditValueIds+'&ylmoney='+ylmoney+'&matchingBorrowQuota='+matchingBorrowQuota,  
  	 			 {}, "get", function(json){
		  	    	...
		  	    	for(var i=0; i<json.length; i++)  
		  	    		{  
		  	    			//var hkrs = Number(json[i].loanMonths)-Number(json[i].loanMonthsSurplus);
	...
	loadingMarkShow();
	var subUrl = ctx+'/matchingcreditor/matchingSubmit?matchingId='+matchingId+'&lendCode='+lendCode+'&creditValueIds='+creditValueIds+'&tjmoneys='+tjmoneys+'&ymyType=1&distributedFlag='+distributedFlag+'&from='+from+'&verTime='+verTime+'&borrowVerTimes='+borrowVerTimes;
	contents_getJsonForHtml(
			subUrl,
			null,
			'post',
