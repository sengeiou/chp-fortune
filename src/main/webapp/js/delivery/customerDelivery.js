	  ...
	  $("#delivery",iframe).click(function(){
	   ...
	   $("#outExcel",iframe).click(function(){
		   BootstrapDialog.confirm('确定要导出EXCEL吗？', function(result){
		         if(result){
		        	 var code = getCustCode();
		        	 $("#id").val(code);
		        	 var url = ctx+"/delivery/customer/outExcel?"+$("#searchForm").serialize();
					 window.location=url;
		         }else{
		        	 return;
		         }
	   ...
	   $("#upload",iframe).click(function(){
			...
			url = '/delivery/customer/showDialog';
			argment = null;
	   ...
	   $("#loadManagerInfo",iframe).click(function(){
		   ...
		   if(ids == 1){
			   ...
			   contents_getJsonForSync(
	  		    		 ...
	  		    		 function(json){
	  		    			 if(json.length == 0){
	  		    				 BootstrapDialog.alert('您所查询的信息不存在！'); 
	  		    			 }else{
