package com.creditharmony.fortune.platform.web;
 ...
 */
@Controller
@RequestMapping(value = "${adminPath}/platformRule")
public class PlatformGotoRuleController extends BaseController{
	...
	public String addPlatformEntity(){
		return "platform/platformDetailed";
	}
	...
	@ResponseBody
	public String checkPlatform(String platformId){
		...
		try {
			...
			return jsonMapper.toJson(BooleanType.FALSE);
		}
		if (ObjectHelper.isEmpty(platformGotoRule)) {
			return jsonMapper.toJson(BooleanType.TRUE);
		}
		return jsonMapper.toJson(BooleanType.FALSE);
	}
}
