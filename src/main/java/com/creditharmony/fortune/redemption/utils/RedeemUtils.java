...
//		System.out.println(DateUtils.formatDate(cycleStart.getTime(), "yyyy-MM-dd"));
//		System.out.println(DateUtils.formatDate(cycleEnd.getTime(), "yyyy-MM-dd"));
	 ...
	 * @return
	 */
	public static ProcessQueryBuilder getQueryBuilder(RedemptionApplyEntity entity) {
		ProcessQueryBuilder queryBuilder = new ProcessQueryBuilder();
		
		if (!ObjectHelper.isEmpty(entity.getCustomerName())) {
			queryBuilder.put("customerName@like", "%"+entity.getCustomerName()+"%");
		}
		if (!ObjectHelper.isEmpty(entity.getCustomerCode())) {
			queryBuilder.put("customerCode@like", "%"+entity.getCustomerCode()+"%");
		}
		if (!ObjectHelper.isEmpty(entity.getLendCode())) {
			queryBuilder.put("applyCode@like", "%"+entity.getLendCode()+"%");
		}
		if (!ObjectHelper.isEmpty(entity.getOrgID())) {
			queryBuilder.put("department", entity.getOrgID().split(","));
		}
		if (!ObjectHelper.isEmpty(entity.getCity())) {
			String arr[] = entity.getCity().split("_");
			if(arr.length>=2){
				queryBuilder.put("city", arr[1]);// 城市查询
			}else{
				queryBuilder.put("province", arr[0]);// 省份查询
			}
		}
		if (!ObjectHelper.isEmpty(entity.getApplyPay())) {
			queryBuilder.put("applyPay", entity.getApplyPay().split(","));
		}
		if (!ObjectHelper.isEmpty(entity.getApplyBillday())) {
			queryBuilder.put("applyBillday", entity.getApplyBillday().split(","));
		}
		if (!ObjectHelper.isEmpty(entity.getApplyLendMoney())) {
			queryBuilder.put("applyLendMoney@>=", entity.getApplyLendMoney().doubleValue());
		}
		if (!ObjectHelper.isEmpty(entity.getApplyLendMoneyTo())) {
			queryBuilder.put("applyLendMoney@<=", entity.getApplyLendMoneyTo().doubleValue());
		}
		if (!ObjectHelper.isEmpty(entity.getProductCode())) {
			queryBuilder.put("productName", entity.getProductCode().split(","));
		}
		return queryBuilder;
	}
