...
/**
 * 提前赎回审批查看导出
 * @Class Name RedeemApprovalExportor
 * @author 陈广鹏
 * @Create In 2016年4月19日
 */
public class RedeemApprovalExportor extends RedeemExportor {

	public RedeemApprovalExportor(String filename) {
		super(filename, "com.creditharmony.fortune.redemption.common.dao.RedemptionHistoryDao.findApprovalCheck");
	}
	...
	public void assembleExcelCell(ResultSet resultSet, Sheet dataSheet)
		...
		String redemptionBmoney;
		String redemptionDemoney;
		String redemptionType;
		
		Row dataRow;
		while (resultSet.next()) {
			Date time = resultSet.getDate("checkTime");
			checkTime = DateUtils.formatDate(time, "yyyy-MM-dd");
			...
			fortuneCenter = resultSet.getString("fortuneCenter");
			...
			redemptionBmoney = resultSet.getString("redemption_bmoney");
			if (StringUtils.isNotEmpty(redemptionBmoney)) {
				redemptionBmoney = StringUtils.doNumFormat(new BigDecimal(redemptionBmoney), "#0.00");
			}
			redemptionDemoney = resultSet.getString("redemption_demoney");
			if (StringUtils.isNotEmpty(redemptionDemoney)) {
				redemptionDemoney = StringUtils.doNumFormat(new BigDecimal(redemptionDemoney), "#0.00");
			}
			...
			dataRow.createCell(0).setCellValue(checkTime);
			...
			dataRow.createCell(12).setCellValue(fortuneCenter);
			dataRow.createCell(13).setCellValue(teamManager);
			dataRow.createCell(14).setCellValue(userName);
			dataRow.createCell(15).setCellValue(applyBillday);
			dataRow.createCell(16).setCellValue(linitDay);
			dataRow.createCell(17).setCellValue(backMoneyDay);
			dataRow.createCell(18).setCellValue(applyAgreementEdition);
			dataRow.createCell(19).setCellValue(redeemCost);
			dataRow.createCell(20).setCellValue(redeemBackDeadline);
			dataRow.createCell(21).setCellValue(redemptionBmoney);
			dataRow.createCell(22).setCellValue(redemptionDemoney);
			dataRow.createCell(23).setCellValue(redemptionType);
			row++;
		}
	...
	public void wrapperHeader(Sheet dataSheet) {
		...
		headerRow.createCell(0).setCellValue("审核通过日期");
		...
		headerRow.createCell(13).setCellValue("团队经理");
		headerRow.createCell(14).setCellValue("理财经理");
		headerRow.createCell(15).setCellValue("账单日");
		headerRow.createCell(16).setCellValue("到期日期");
		headerRow.createCell(17).setCellValue("回款日期");
		headerRow.createCell(18).setCellValue("协议版本");
		headerRow.createCell(19).setCellValue("扣费标准");
		headerRow.createCell(20).setCellValue("回款期限");
		headerRow.createCell(21).setCellValue("回款金额");
		headerRow.createCell(22).setCellValue("扣费金额");
		headerRow.createCell(23).setCellValue("赎回类型");
	}
